...
import { updateTripAction } from '@/actions/dashboard/trips/trips';
import type { StopFormRow, TripDetailsDTO, TripDTO } from '@/types/dashboard/trips';
...
export default function EditTripDialog({ visible, onHide, trip, stations }: Props) {
  const stationOptions = useMemo(
    () => stations.map(s => ({ label: s.name, value: s.code })),
    [stations]
  );

  const [details, setDetails] = useState<TripDetailsDTO | null>(null);
  ...
  useEffect(() => {
    async function loadDetails(id: number) {
      ...
      setLoading(true);
      try {
        const res = await fetch(`${base}/trips/${id}`, { method: 'GET', credentials: 'include' });
        if (res.ok) {
          const data: TripDetailsDTO = await res.json();
          setDetails(data);
        }
      } finally {
        setLoading(false);
  ...
  useEffect(() => {
    ...
    setTrainNo(details.trainNo || '');
    setDays([...details.days]);
    setStartDate(details.startDate ? new Date(details.startDate + 'T00:00:00Z') : null);
    setEndDate(details.endDate ? new Date(details.endDate + 'T00:00:00Z') : null);
    setStops(
      (details.stops || []).map(s => ({
        ...
        >
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="text-sm text-neutral-600">
              Перша зупинка не має прибуття, остання — не має відправлення.
            </div>
          ...
          <div className="flex items-center justify-end gap-2 pt-2">
            ...
            <Button type="submit" label={loading ? 'Зачекайте' : 'Зберегти'} icon="pi pi-check" />
          </div>
        </form>
      )}
    </Dialog>
